...
**Gegner-Pool (M32):** Aktiviert der Host beim Erstellen `showOpponentPool`, liefert das Lobby-DTO je Spieler den Wert `rollsInPool`. Die Spiel-Topbar zeigt im Strategy-Modus den eigenen Pool und die Pools der Mitspieler (bei 2: `ich / Gegner`, bei 3+: `ich / A · B · …`). Das Frontend lädt den Wert ohne Polling — nur bei Spielstart und nach jeder eigenen Eintragung. Persistiert als Session-Flag `show_opponent_pool` (Default `false`).
