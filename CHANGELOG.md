...
### Changed
- **Startscreen-Motive moderner und groesser:** Die vier Hauptkarten (`Multiplayer`, `Einzelspiel`, `Statistik`, `Einstellungen`) nutzen neue inline-SVG-Motive mit staerkerem Game-Dashboard-Charakter: Duell-Zettel, Fokus-Wuerfel, Pokal/Balken und Control-Panel. Die Motive wurden groesser gesetzt, bleiben aber per Max-Hoehen und Small-Viewport-Regeln im Gesamtbild der Kacheln. Dateien: `frontend/components/HomeBentoGrid.tsx`, `frontend/app/globals.css`
- **App-Footer in TestFlight hoeher und lesbarer:** Die feste Footer-Zeile ist nun etwa doppelt so hoch, die Legal-/Home-Links sind groesser gesetzt und erhalten dezente runde Glyphen-Badges (`H`, `D`, `I`, `?`) statt bunter Icons. Weiterhin kein `safe-area-inset-bottom`, damit iOS die Links nicht nach oben schiebt. Dateien: `frontend/components/AppLegalFooter.tsx`, `frontend/app/globals.css`
