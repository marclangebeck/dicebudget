...
### Fixed
- **Startscreen auf iOS kompakter:** Auf kurzen iPhone-/iOS-Viewports konnten die kleinen rechten Kacheln (`Statistik`, `Einzelspiel`) zu wenig Platz fuer Motiv und Text haben; ausserdem wurde `Raum beitreten` unten durch die knappe Hoehenverteilung/den Footer angeschnitten. Die Home-Kacheln, Poster-Motive, Join-Box, Header und Footer haben jetzt kompaktere Grundmasse sowie einen `max-height: 760px`-Kompaktmodus. Datei: `frontend/app/globals.css`
