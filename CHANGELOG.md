...
### Added
- **Spielanalyse (Head-to-Head):** Nach Multi-Abschluss (nach Pool-Endspiel bzw. direkt ohne Pool-Endspiel) optional „Spielanalyse“ — abgeleitete Kennzahlen (Attribution, Pool-Effektivität, Yatzy, entscheidender Block/Feld), nicht Zettel-Duplikat. Solo analog. Nachträglich unter `/stats/pairing` pro App-Runde. Backend: `GET /sessions/invite/:code/match-analysis`, Domain `matchAnalysis.ts`. Vorerst 2 Spieler; Struktur `allPlayers` für x-Spieler vorbereitet
