...
import { UPPER_BONUS_POINTS } from "@/lib/gameScoring";
import { FIELD_LABELS, SHEET_ROWS } from "@/lib/labels";
import type {
...
const FIELD_TYPES_PER_GAME: FieldTypeId[] = SHEET_ROWS.filter(
  (row): row is { kind: "field"; fieldType: FieldTypeId } => row.kind === "field",
).map((row) => row.fieldType);

function fieldLabel(fieldType: FieldTypeId): string {
  return FIELD_LABELS[fieldType] ?? fieldType;
}

type AnalysisRun = Pick<
  RunDto,
  "gameCount" | "useStrategyRules" | "totalScore" | "totalRollsUsed" | "rollsInPool" | "extraYatzyCount" | "games"
>;

type PlayerRunMetrics = PlayerRunMetricsDto;
type HeadToHeadAnalysis = HeadToHeadAnalysisDto;
...
function buildTips(
  ...
  if (h2h?.decisiveFieldType && h2h.decisiveFieldDiff < -10) {
    tips.push({
      ...
      title: `${fieldLabel(h2h.decisiveFieldType as FieldTypeId)} verbessern`,
